...
public class DirectoryAdapter extends RecyclerView.Adapter<DirectoryAdapter.ViewHolder> implements Filterable {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (!filesListFiltered.get(position).isDirectory()) {
            ...
            changeFileIcon(holder, filesListFiltered.get(position));
        ...
        holder.itemView.setOnClickListener((v) -> {
            if (filesListFiltered.get(finalPosition).isDirectory()) {
                onFilesClickListener.onClicked(filesListFiltered.get(finalPosition));
            } else {
                if (config.isSelectMultiple()) {
                    if (selected.contains(String.valueOf(finalPosition))) {
                        selected.remove(String.valueOf(finalPosition));
                        holder.rg_selected.setVisibility(View.GONE);
                        holder.rl_file_root.setBackgroundColor(this.backgroundTint);
                    } else {
                        selected.add(String.valueOf(finalPosition));
                        holder.rg_selected.setVisibility(View.VISIBLE);
                        holder.rl_file_root.setBackgroundColor(this.selectionTint);
                    }
                } else {
                    ...
                    if (selected.size() == 0) {
                        selected.add(0, String.valueOf(finalPosition));
                    }
                    /* if item already selected then remove it */
                    else if (selected.get(0).equals(String.valueOf(finalPosition))) {
                        selected.remove(0);
                    }
                ...
                notifyDataSetChanged();
                onFilesClickListener.onFileSelected(filesListFiltered.get(finalPosition));
            }
        });
    }
...
//           e.printStackTrace();
    ...
    private void changeFileIcon(ViewHolder holder, DirectoryModel directoryModel) {
        //TODO add photo preview here

        File file = new File(directoryModel.getPath());

        try {

            String fileName = directoryModel.getName();
            String extension = fileName.substring(fileName.lastIndexOf("."));
            if (extension.toLowerCase().contains("pdf")) {
                holder.item_icon.setColorFilter(Color.argb(255, 255, 255, 255));
                holder.item_icon.setImageResource(R.drawable.unicorn_ic_pdf);
            } else if (
                    extension.toLowerCase().contains("png") ||
                            extension.toLowerCase().contains("jpg") ||
                ...
                holder.item_icon.setColorFilter(Color.argb(0, 255, 255, 255));
